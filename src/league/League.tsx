...
class League extends React.Component<ILeagueProps, ILeagueState> {
	...
	constructor(props: ILeagueProps) {
		...
		const gameID = matchGame && matchGame.length ? matchGame[0].split('/')[1] : false;
		...
		return (
			...
			>
				<Menu.Item key="dashboard">
					<DashboardOutlined /> <Link to={`${match.url}`}>League Summary</Link>
				</Menu.Item>
				<SubMenu
					key="gamesSub"
					title={
				...
				>
					{leagueGames.map((game, i) => (
						<Menu.Item key={game.gameID.toString()}>
							<Link to={`${match.url}${routes.GAME}/${game.gameID}`}>Game {game.gameID}</Link>
						</Menu.Item>
					))}
