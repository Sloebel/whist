...
import { IPlayerHand, ICardsFromTo, ICardsPosition, IRoundData, IHandState } from '../models/IGameModel';
...
export const calcIfRoundFell = (roundData: IRoundData): boolean => {
	...
	for (let i = 0; i < 4; i++) {
		...
		if (typeof roundData[bid] !== 'number' || typeof roundData[won] !== 'number') {
			didRoundFell = false;
			break;
...
export const getDevicePlayerIndex = (players: IPlayer[]): number => {
	return players.findIndex(player => player.key === fire.auth().currentUser?.uid);
};
...
export const CARD_MARKER_HEIGHT = 188;
...
export const getToWinnerCalc = (direction: ThrownCardType, markerTop: number, markerWidth: number) => {
	switch (direction) {
		case 'right':
			return {
				to: {
...
export const calculateHandWinner = (roundData: IRoundData) => {
	...
	const trumpCards = thrownCards.filter(
		thrown => (thrown.card.split('-')[1] as CARDS_SHORT) === CardsType[trump as CARDS]
	);

	if (trumpCards.length) {
		const thrownSorted = trumpCards.sort((a, b) => +b.card.split('-')[0] - +a.card.split('-')[0]);
